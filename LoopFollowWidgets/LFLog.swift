...
// Duplicated verbatim from LoopFollowWatch/LFLog.swift so the widget extension
// target can read/write the same App Group UserDefaults (group.loopfollow.shared).
// The two copies use identical keys and an identical Event shape so snapshots
// from both targets are visible in the diagnostics view.
//
// Writes:
